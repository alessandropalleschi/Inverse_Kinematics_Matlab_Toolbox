function [q] = reverse_priority_pos_or_limits(N, Ts, iter_num, q_0, x_des,xd_des, unil_constr, x_cons, param_vect,robot, ee)
%{
===========================================================================
...
e_p = 1;
e_o = 1;
while(norm(e_p)>1e-6 || norm(e_o)>1e-6)    % specific part
        ...
        % actual x
        x(:,k) = {q(:,k-1); q(:,k-1);{T(1:3,4);T(1:3,1:3)}};
...
%         x_des_prev = x_des(:,k-1);
        ...
        for p = N:-1:1     % e.g.: 3 2 1
            ...
            if unil_constr(p)
                ...
                x_goal = x_des(:,end);
                e_o = eo_using_quat(x_cur{p}{2}, x_goal{p}{2});
                e_p = x_cur{p}{1} - x_goal{p}{1};
                e = -[e_p;e_o];
            end
